...
class MigrationTest < ActiveRecord::TestCase
  ...
  def test_any_migrations
    migrator = ActiveRecord::MigrationContext.new(MIGRATIONS_ROOT + "/valid", ActiveRecord::Base.connection.schema_migration)

    assert_predicate migrator, :any_migrations?

    migrator_empty = ActiveRecord::MigrationContext.new(MIGRATIONS_ROOT + "/empty", ActiveRecord::Base.connection.schema_migration)

    assert_not_predicate migrator_empty, :any_migrations?
  end

  def test_migration_version
    migrator = ActiveRecord::MigrationContext.new(MIGRATIONS_ROOT + "/version_check", ActiveRecord::Base.connection.schema_migration)
    assert_equal 0, migrator.current_version
